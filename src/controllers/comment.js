  ...
  async postComment(req, res) {
    ...
    try {
      ...
      const [blog, user] = await Promise.all([
        BlogScheam.findById(blogId, {}, {}), // find에도 꼭 session 을 넣을것!
        UserScheam.findById(userId, {}, {}),
      ]);
      ...
      // blog에는 find 할때 session을 넣어줬기때문에 내장되있어서 필요없음
      // await Promise.all([comment.save({}), blog.save()]);
      // });

      await Promise.all([
        comment.save(),
        BlogScheam.updateOne(
          { _id: blogId },
          { $inc: { commentCount: 1 }, $push: { comment: { $each: [comment], $slice: -3 } } }
        ),
      ]);
